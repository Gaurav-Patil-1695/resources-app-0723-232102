...
function generateMockProducts(count = 48) {
  return Array.from({ length: count }, (_, i) => ({
    id: i + 1,
    slug: `product-${i + 1}`,
    name: `Product ${i + 1}`,
    brand: MOCK_FILTERS.brands[i % MOCK_FILTERS.brands.length],
    price: parseFloat((9.99 + i * 3.5).toFixed(2)),
    taxInclusivePrice: parseFloat((9.99 + i * 3.5) * 1.1).toFixed(2),
    rating: parseFloat((3 + (i % 3) * 0.5).toFixed(1)),
    reviewCount: 10 + i * 3,
    image: null,
    inStock: i % 7 !== 0,
  }));
...
const ALL_PRODUCTS = generateMockProducts(48);
    ...
    >
      ...
      <div style={sectionStyle}>
        ...
        {expandedSections.price && (
          <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {MOCK_FILTERS.priceRanges.map(range => (
              <li key={range.label}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '14px', color: tokens.colorBody }}>
                  <input
                    type='radio'
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              <p style={{ fontSize: '14px', color: tokens.colorMuted, margin: 0 }}>
                <strong style={{ color: tokens.colorInk }}>{totalCount}</strong> product{totalCount !== 1 ? 's' : ''} found
              </p>
