...
const MOCK_BRANDS_MAP = {
  1: { name: 'CoreWear', slug: 'corewear', description: 'Premium everyday basics.', website: 'https://corewear.com', active: true },
  2: { name: 'SpeedFit', slug: 'speedfit', description: 'Performance sportswear.', website: 'https://speedfit.io', active: true },
  3: { name: 'NovaCraft', slug: 'novacraft', description: '', website: '', active: false },
};
...
export default function AdminBrandEdit() {
  ...
  useEffect(() => {
    setTimeout(() => {
      const brand = MOCK_BRANDS_MAP[id] || { name: '', slug: '', description: '', website: '', active: true };
      setForm({ ...brand });
      setLoading(false);
    }, 600);
  }, [id]);
  ...
  if (loading) {
    return (
      <div style={styles.page}>
        <div style={{ ...styles.skeletonBlock, width: '120px', height: '20px', marginBottom: '20px' }} />
        <div style={{ ...styles.skeletonBlock, width: '200px', height: '32px', marginBottom: '24px' }} />
        <div style={styles.card}>
          {[1, 2, 3].map(i => <div key={i} style={styles.skeletonBlock} />)}
        </div>
      </div>
    );
  }
